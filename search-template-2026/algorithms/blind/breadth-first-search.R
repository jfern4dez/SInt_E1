...
# =========================================================================
# Breadth-First Search (BFS) expands the shallowest node first (FIFO order).
# It explores the search tree level by level and returns the solution with the
# smallest number of actions (optimal in steps when all step costs are equal).
#
# Arguments:
#   problem       : The problem object (must contain state_initial, actions_possible, etc.)
...
#
# EFFICIENCY NOTE:
# Although 'frontier' is a list/queu of nodes, checking membership in a 
# list is expensive (O(n)). Therefore, we use an auxiliary hash-based set 
# (frontier_set) to test membership in O(1). This redundancy keeps the algorithm  
...
breadth.first.search <- function(problem,
  ...
  while (count <= max_iterations) {
    ...
    if (length(successor_nodes) > 0) {
      ...
      # Add successors to the END of the frontier (FIFO)
      if (length(successor_nodes) > 0) {
